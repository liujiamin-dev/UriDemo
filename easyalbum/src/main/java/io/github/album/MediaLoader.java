...
final class MediaLoader {
    ...
    private static boolean refresh(final QueryController controller) {
        Cursor cursor = ContentResolverCompat.query(
                ...
                CONTENT_URI, ID_PROJECTION, TYPE_SELECTION, null, null, (CancellationSignal) null
        );
        if (cursor == null) {
            return false;
        }
    ...
    private static List<MediaData> query(String idSelection) {
        ...
        Cursor cursor = ContentResolverCompat.query(Utils.appContext.getContentResolver(), CONTENT_URI, PROJECTIONS, selection, null, null, (CancellationSignal) null);
        if (cursor == null) {
            return null;
        }
        List<MediaData> list;
